...
Many electronics hobbyists begin with the Arduino ecosystem. My situation was no different.

My first project was an iPhone camera-tripod mount that could pivot via a motor. Early iterations used an [Arduino Nano 33 BLE](https://store.arduino.cc/products/arduino-nano-33-ble), a [servo motor](https://www.amazon.com/gp/product/B09BV5D7MD/), and 4x AA batteries.
...
When writing firmware for this project it made sense to start with the Arduino hardware, IDE, and SDK.

The code for communicating over Bluetooth and controlling the servo fit into a few hundred line `.ino` sketch file.
The wiring was pretty straightforward and only needed a few pins connected.

In future projects, my electronics requirements became increasingly complex. The [Bitclock](https://bitclock.io) project
...
By using a custom PCB, the device could have a much smaller electronics footprint that would fit snugly into an enclosure.
Manufacturing the product at scale would also be simplified through PCB assembly services
like 🇨🇳 [JLCPCB](https://jlcpcb.com/), [PCBWay](https://www.pcbway.com/), or 🇺🇸 [MacroFab](https://www.macrofab.com/).

I should add that one of my main goals when starting [Bitclock](https://bitclock.io) was to learn custom PCB design,
so landing there was somewhat predetermined!
...
An early question when designing [Bitclock](https://bitclock.io) was which microcontoller to use.

I settled on the [ESP32-S3](https://www.espressif.com/en/products/socs/esp32-s3) module mounted to the top of the PCB. These ESP32 SoC (system-on-a-chip) modules are popular for a few reasons:
...
I needed to render graphics for a display, communicate over bluetooth + Wi-Fi, communicate with sensors.
This multi-tasking needed to happen all on a single core, with memory and flash constraints.
...
In this post, I share the developer environment used for the [Bitclock](https://bitclock.io) project that I've found to work well.

### ESP-IDF vs Arduino SDK

For a simple "blinking LED" app, the Arduino SDK can be the fastest way to get started.
The generic SDK allows code to be (mostly) hardware-agnostic via per-hardware ports known as [**Arduino Cores**](https://github.com/arduino/ArduinoCore-API).

Each Arduino Core implements the basic APIs like [Serial](https://www.arduino.cc/reference/en/language/functions/communication/serial/)
and a collection of Arduino [libraries](https://www.arduino.cc/reference/en/libraries/) like [Wire](https://www.arduino.cc/reference/en/language/functions/communication/wire/) &
[WiFi](https://www.arduino.cc/reference/en/libraries/wifi/) for their respective hardware platforms. In the case of ESP32 devices, Espressif
maintains the [arduino-esp32](https://github.com/espressif/arduino-esp32) core implementation.

Under the hood, the [arduino-esp32](https://github.com/espressif/arduino-esp32) core is built on the [**ESP-IDF framework**](https://github.com/espressif/esp-idf).
Whereas Arduino is a generic APIs for many types of embedded devices, ESP-IDF is scoped more narrowly and can make API assumptions for Espressif hardware.
...
Let's consider the case of enabling Wi-Fi. Using Arduino libraries, you would do the following:
